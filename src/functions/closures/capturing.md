...
fn main() {
    ...
    // `int`에서 사용되는 `count`은 mutable하기 때문에(`&mut count`) 
    // 클로저 본인도 `mut` 선언이 필요합니다.
    // 해당 클로저의 호출은 클로저 스스로를 변형시킵니다.
    let mut inc = || {
        count += 1;
        println!("`count`: {}", count);
    };
    ...
    // `int` 클로저는 여전히 mutable `count`를 사용하고 있습니다.
    ...
    // 반면 non-copy 타입은 즉각적으로 값을 클로저로 이동시킵니다.
    let consume = || {
